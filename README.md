...
- ✅ 可选显示每个插座的功率和电流传感器
- ✅ 可配置轮询间隔（默认 30 秒），支持在"选项"中随时修改，无需删除重加
- ✅ 本地轮询，无需云服务
- ✅ 支持多台 PDU 设备（同一主机自动防重复添加）
- ✅ 添加时自动验证连接；设备暂时离线时自动重试加载
- ✅ 插座能耗统计（今日/昨日用电）持久化保存，重启不丢数据
- ✅ **[优化]** 解析逻辑重构，更加健壮
- ✅ **[优化]** 增强会话管理与错误恢复
...
#### GWGJ PDU（GWGJ PDU 网关/客户端）

支持两种模式：
1. **服务器模式**：运行一个 TCP 服务器，接收 GWGJ 协议 PDU 的主动连接（无需 MQTT）。
2. **客户端模式**：通过 HTTP 协议主动轮询特定的 PDU 设备（适用于特定型号）。

**主要功能:**
- ✅ 监听 TCP 端口并解析 GWGJ 协议数据
- ✅ 支持 HTTP 轮询模式，主动获取设备状态
...
**设备配置文件位置:** `<HA配置目录>/.storage/gwgj_pdu_data/<entry_id>/devices.json`（设备自动注册后生成）
...
- `sensor.pdu_{device_id}_总能耗`: 总能耗（kWh）
